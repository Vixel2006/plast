...
PYBIND11_MODULE(_plast_cpp_core, m) {
    ...
    py::class_<plast::tensor::Tensor>(m, "Tensor")
        ...
        .def("__repr__", [](const plast::tensor::Tensor &t) {
            std::stringstream ss;
            ss << "Tensor(shape=" << py::cast(t.shape()) << ", dtype=" << py::cast(t.dtype()) << ", device=" << py::cast(t.device()) << ")";
            return ss.str();
        });
    ...
    py::class_<plast::ops::BaseOperation, std::shared_ptr<plast::ops::BaseOperation>>(m, "BaseOperation")
        .def_property_readonly("name", &plast::ops::BaseOperation::name);

    // Bind specific operations (e.g., AddOperation, SubOperation)
    py::class_<plast::ops::AddOperation, plast::ops::BaseOperation, std::shared_ptr<plast::ops::AddOperation>>(m, "AddOperation")
        .def(py::init<>());

    py::class_<plast::ops::SubOperation, plast::ops::BaseOperation, std::shared_ptr<plast::ops::SubOperation>>(m, "SubOperation")
        .def(py::init<>());
    ...
    m.def("zeros", &plast::ops::init::zeros,
          py::arg("shape"), py::arg("dtype"), py::arg("device"));
    m.def("ones", &plast::ops::init::ones,
          py::arg("shape"), py::arg("dtype"), py::arg("device"));
    m.def("randn", &plast::ops::init::randn,
          py::arg("shape"), py::arg("dtype"), py::arg("device"), py::arg("seed"));
    m.def("uniform", &plast::ops::init::uniform,
          py::arg("shape"), py::arg("dtype"), py::arg("device"), py::arg("low"), py::arg("high"));
    m.def("from_data", [](py::array data_array, const std::vector<size_t>& shape, plast::core::DType dtype, plast::core::DeviceType device) {
        ...
        if (buf_info.format != py::format_descriptor<float>::format() &&
            ...
            throw std::runtime_error("Unsupported data type for from_data. Only float and int are supported.");
        }
    ...
    m.def("add_op_node", [](std::shared_ptr<plast::graph::Node> lhs, std::shared_ptr<plast::graph::Node> rhs) {
        auto op = std::make_shared<plast::ops::AddOperation>();
        return std::make_shared<plast::graph::Node>(op, std::vector<std::shared_ptr<plast::graph::Node>>{lhs, rhs});
    }, py::arg("lhs"), py::arg("rhs"));

    // This function would be called from Python's `plast.ops.sub`
    m.def("sub_op_node", [](std::shared_ptr<plast::graph::Node> lhs, std::shared_ptr<plast::graph::Node> rhs) {
