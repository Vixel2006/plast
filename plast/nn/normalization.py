...
class BatchNorm1d(Module):
    def __init__(self, num_features, eps=1e-5, momentum=0.1, affine=True, track_running_stats=True, device=Device.CPU):
        super().__init__()
        ...
        self.running_mean = tensor(np.zeros(num_features, dtype=np.float32), device=device, persistent=True)
        self.running_var = tensor(np.ones(num_features, dtype=np.float32), device=device, persistent=True)
    ...
    def forward(self, input):
        ...
        if ndim == 3:
            ...
            out_4d = F.batch_norm(input_4d, self.running_mean, self.running_var, self.weight, self.bias,
