...
__global__ void mean_grad_kernel(const float* out_grad, float* in_grad, int n_in,
                                 const int* in_shape, const int* in_strides, int in_ndim,
                                 const int* out_strides, int out_ndim, int reduced_dim)
{
    ...
    // Get in_grad offset
    int in_grad_offset = 0;
    for (int d = 0; d < in_ndim; ++d)
    {
        in_grad_offset += in_coords[d] * in_strides[d];
    }

    // Get out_grad offset
    int out_grad_offset = 0;
    int out_d = 0;
    for (int d = 0; d < in_ndim; ++d)
    {
        ...
        out_grad_offset += coord * out_strides[out_d];
        out_d++;
    }

    float reduction_size = (float) in_shape[reduced_dim];
    in_grad[in_grad_offset] += out_grad[out_grad_offset] / reduction_size;
}
...
void mean_grad_op_cuda(Tensor* out, Tensor** prev, int n_prev, void* extras)
{
    ...
    mean_grad_kernel<<<num_blocks, num_threads_per_block>>>(
        out->grad->data->data, a->grad->data->data, N, d_in_shape, d_in_strides, a->ndim,
        d_out_strides, out->ndim, axis);
