                                          ...
                                          int out_ndim)
{
    ...
    for (int i = idx; i < n; i += stride)
    {
        ...
        atomicAdd(&prev_grad[prev_offset], out_grad[out_offset] * out_data[out_offset]);
    }
}
...
void exp_grad_op_cuda(Tensor* out, Tensor** prev, int n_prev, void* extras)
{
    ...
    if (is_contiguous(a) && is_contiguous(out))
    {
        exp_grad_kernel<<<num_blocks, num_threads_per_block>>>(
            out->grad->data->data, out->data->data, a->grad->data->data, N);
    }
    else
    {
        ...
        noncontig_exp_grad_kernel<<<num_blocks, num_threads_per_block>>>(
            out->grad->data->data, out->data->data, a->grad->data->data, N, d_prev_shape,
