                                          ...
                                          int out_ndim)
{
    ...
    for (int i = idx; i < n; i += stride)
    {
        ...
        atomicAdd(&prev_grad[prev_offset], out_grad[out_offset] / (prev_data[prev_offset] + 1e-7f));
    }
}
...
void log_grad_op_cuda(Tensor* out, Tensor** prev, int n_prev, void* extras)
{
    ...
    {
        log_grad_kernel<<<num_blocks, num_threads_per_block>>>(out->grad->data->data, a->data->data,
                                                               a->grad->data->data, N);
    }
    else
    {
        ...
        noncontig_log_grad_kernel<<<num_blocks, num_threads_per_block>>>(
            out->grad->data->data, a->data->data, a->grad->data->data, N, d_prev_shape,
            d_prev_strides, a->ndim, d_out_shape, d_out_strides, out->ndim);
