...
void add_cpu_forward_float_kernel(const float *a_data, const u64 *a_strides, const u64 *a_shape,
                                  u64 a_ndim, const float *b_data, const u64 *b_strides,
                                  const u64 *b_shape, u64 b_ndim, float *c_data,
...
void add_cpu_backward_float_kernel(const float *dout_data, const u64 *dout_strides,
                                   const u64 *dout_shape, u64 dout_ndim, float *da_data,
                                   const u64 *da_strides, const u64 *da_shape, u64 da_ndim,
                                   float *db_data, const u64 *db_strides, const u64 *db_shape,
                                   u64 db_ndim) {
  u64 num_elements = 1;
...
void add_cpu_backward(Tensor **inputs, const Tensor *output, KernelParams params) {
  ...
  case FLOAT32:
    add_cpu_backward_float_kernel(
        ...
        output->grad->ndim, a->requires_grad ? (float *)a->grad->data : NULL, a->strides, a->shape,
        a->ndim, b->requires_grad ? (float *)b->grad->data : NULL, b->strides, b->shape, b->ndim);
    break;
  default:
    break;
  }
