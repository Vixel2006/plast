...
void tan_cpu_forward_float_non_contig_kernel(const float *a_data, const u64 *a_strides,
...
void tan_cpu_backward_float_non_contig_kernel(const float *dout_data, const u64 *dout_strides,
...
#pragma omp parallel for private(coords)
  for (u64 i = 0; i < num_elements; ++i) {
    ...
    if (da_data)
      da_data[da_offset] +=
          dout_data[dout_offset] / (cosf(a_data[a_offset]) * cosf(a_data[a_offset]));
  }
}
...
void tan_cpu_forward(const Tensor **inputs, Tensor *output, KernelParams params) {
  ...
  if (is_contiguous(a) && is_contiguous(output)) {
    switch (a->dtype) {
    case FLOAT32:
      tan_cpu_forward_float_contig_kernel((const float *)a->data, (float *)output->data,
                                          num_elements);
      break;
    default:
      break;
    ...
    switch (a->dtype) {
    case FLOAT32:
      tan_cpu_forward_float_non_contig_kernel((const float *)a->data, a->strides,
