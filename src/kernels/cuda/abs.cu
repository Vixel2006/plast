...
__global__ void abs_cuda_forward_float_contig_kernel(const float *a, float *c, u64 num_elements) {
...
__global__ void abs_cuda_backward_float_contig_kernel(const float *dout, const float *a, float *da,
                                                      u64 num_elements) {
  u64 idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < num_elements) {
    if (da) {
      float grad_multiplier = (a[idx] > 0) ? 1.0f : ((a[idx] < 0) ? -1.0f : 0.0f);
      da[idx] += dout[idx] * grad_multiplier;
    }
  }
}

__global__ void abs_cuda_forward_float_non_contig_kernel(const float *a_data, const u64 *a_strides,
