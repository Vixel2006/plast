...
__global__ void div_cuda_forward_float_contig_kernel(const float *a, const float *b, float *c,
                                                     u64 num_elements) {
...
__global__ void div_cuda_backward_float_contig_kernel(const float *dout, const float *a,
...
__global__ void div_cuda_backward_float_non_contig_kernel(
    ...
    const float *a_data, const u64 *a_strides, const u64 *a_shape, u64 a_ndim, const float *b_data,
    const u64 *b_strides, const u64 *b_shape, u64 b_ndim, float *da_data, const u64 *da_strides,
    const u64 *da_shape, u64 da_ndim, float *db_data, const u64 *db_strides, const u64 *db_shape,
    u64 db_ndim, u64 num_elements) {
  u64 linear_idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (linear_idx < num_elements) {
    u64 coords[MAX_NDIM];
    ...
    if (da_data) {
      ...
      atomicAdd(&da_data[da_offset], dout_val / b_val);
    }
    if (db_data) {
      u64 db_offset = cuda_get_offset_broadcast(coords, dout_ndim, db_strides, db_shape, db_ndim);
      atomicAdd(&db_data[db_offset], -dout_val * a_val / (b_val * b_val));
    }
  }
}

void div_cuda_forward(const Tensor **inputs, Tensor *output, KernelParams params) {
  ...
  if (is_contiguous(a) && is_contiguous(b) && is_contiguous(output) && a->ndim == b->ndim &&
      a->ndim == output->ndim) {
    // Check if shapes match exactly (no broadcasting)
  ...
  // Fallback to non-contiguous / broadcasting kernel
  switch (a->dtype) {
  case FLOAT32:
...
void div_cuda_backward(Tensor **inputs, const Tensor *output, KernelParams params) {
  const Tensor *a = inputs[0];
  const Tensor *b = inputs[1];
  u64 num_elements = numel(output);
  ...
  // For simplicity and correctness with broadcasting (atomicAdd),
  // we use the non-contiguous kernel as the primary path for now.
  // Many real-world ops use specialized kernels for power-of-two broadcasting,
  // etc.
  ...
  case FLOAT32:
    div_cuda_backward_float_non_contig_kernel<<<grid_size, block_size>>>(
        ...
        output->grad->ndim, (const float *)a->data, a->strides, a->shape, a->ndim,
        (const float *)b->data, b->strides, b->shape, b->ndim,
        a->requires_grad ? (float *)a->grad->data : NULL, a->grad ? a->grad->strides : NULL,
        a->grad ? a->grad->shape : NULL, a->grad ? a->grad->ndim : 0,
        b->requires_grad ? (float *)b->grad->data : NULL, b->grad ? b->grad->strides : NULL,
        b->grad ? b->grad->shape : NULL, b->grad ? b->grad->ndim : 0, num_elements);
    break;
  default:
    break;
  }
