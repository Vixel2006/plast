...
__global__ void min_cuda_forward_float_contig_kernel(const float *a, float *c, u64 num_elements) {
...
template <u64 block_size>
__global__ void min_cuda_forward_float_noncontig_kernel(const float *a_data, const u64 *a_shape,
...
__global__ void min_cuda_forward_float_dim_contig_kernel(const float *a_data, const u64 *a_shape,
...
__global__ void min_cuda_backward_float_dim_contig_kernel(
    ...
    const float *a_data_fwd, const float *c_data_fwd, const u64 *da_shape, const u64 *da_strides,
...
__global__ void min_cuda_backward_float_contig_kernel(const float *dc, float *da,
                                                      const float *a_data_fwd,
                                                      const float *c_data_fwd, u64 num_elements) {
...
__global__ void min_cuda_backward_float_noncontig_kernel(const float *dc, float *da,
                                                         const float *a_data_fwd,
                                                         const float *c_data_fwd, u64 *a_shape,
    ...
    if (is_contiguous(a)) {
      ...
      case FLOAT32:
        min_cuda_backward_float_dim_contig_kernel<<<grid_size, block_size>>>(
            ...
            a->shape, a->strides, a->ndim, dim, reduce_size, keepdim);
        break;
      default:
        break;
      }
