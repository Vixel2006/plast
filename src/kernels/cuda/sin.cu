...
__global__ void sin_cuda_forward_float_contig_kernel(const float *a, float *c, u64 num_elements) {
...
__global__ void sin_cuda_backward_float_contig_kernel(const float *dout, const float *a, float *da,
                                                      u64 num_elements) {
  u64 idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < num_elements) {
    if (da)
...
__global__ void sin_cuda_forward_float_non_contig_kernel(const float *a_data, const u64 *a_strides,
