...
__global__ void sub_cuda_forward_float_contig_kernel(const float *a, const float *b, float *c,
                                                     u64 num_elements) {
...
__global__ void sub_cuda_backward_float_contig_kernel(const float *dout, float *da, float *db,
                                                      u64 num_elements) {
  u64 idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < num_elements) {
    if (da)
