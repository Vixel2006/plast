...
__global__ void broadcasted_pow_kernel(const float* a, const float* b, float* out, const int n,
                                     ...
                                     int out_ndim)
{
    ...
    for (int i = idx; i < n; i += stride)
    {
        ...
        out[out_offset] = powf(fmaxf(a[a_offset], 0.0f) + 1e-9f, b[b_offset]);
    }
}
...
extern "C" void pow_op_cuda(Tensor* a, Tensor* b, Tensor* out)
{
    ...
    else
    {
        ...
        broadcasted_pow_kernel<<<num_blocks, num_threads_per_block>>>(
