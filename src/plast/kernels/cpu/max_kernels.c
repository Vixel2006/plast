                                                ...
                                                const size_t* input_strides)
{
    ...
    float max_val = -FLT_MAX; // Initialize with a small value

    for (size_t i = 0; i < total_elements; ++i)
    {
        size_t physical_idx = get_physical_index(i, input_shape, input_ndim, input_strides);
        if (input_data[physical_idx] > max_val)
        {
            max_val = input_data[physical_idx];
        }
    }
    output_data[0] = max_val;
}
                                                ...
                                                const size_t* input_strides)
{
    ...
    int32_t max_val = INT32_MIN; // Initialize with a small value
                                               ...
                                               int dim)
{
    ...
    for (size_t out_flat_idx = 0; out_flat_idx < output_total_elements; ++out_flat_idx)
    {
        ...
        for (size_t k = 0; k < reduction_size; ++k)
        {
            ...
            if (input_data[input_physical_idx] > current_max)
            {
                current_max = input_data[input_physical_idx];
            }
        }
        output_data[out_flat_idx] = current_max;
    }
