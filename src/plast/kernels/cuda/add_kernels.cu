...
__global__ void add_cuda_kernel_float_kernel(float* out, const float* in1, const float* in2, size_t num_elements) {
...
extern "C" void plast_cuda_add_kernel_float(float* out, const float* in1, const float* in2, size_t num_elements) {
    int blockSize = 256;
...
__global__ void add_cuda_kernel_int32_kernel(int32_t* out, const int32_t* in1, const int32_t* in2, size_t num_elements) {
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < num_elements) {
        out[idx] = in1[idx] + in2[idx];
    }
...
extern "C" void plast_cuda_add_kernel_int32(int32_t* out, const int32_t* in1, const int32_t* in2, size_t num_elements) {
