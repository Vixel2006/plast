...
__global__ void contig_sub_kernel_int32(const int32_t* a, const int32_t* b, int32_t* out, const int n) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n) {
        out[idx] = a[idx] - b[idx];
    }
...
extern "C" void plast_cuda_sub_kernel_float(float* out, const float* in1, const float* in2, size_t num_elements) {
...
extern "C" void plast_cuda_sub_kernel_int32(int32_t* out, const int32_t* in1, const int32_t* in2, size_t num_elements) {
    int num_threads_per_block = 256;
    int num_blocks = (num_elements + num_threads_per_block - 1) / num_threads_per_block;
...
// Note: The original sub.cu had broadcasted kernels.
// For simplicity in this refactoring, we are currently assuming contiguous, same-shaped inputs.
// Broadcasting logic will need to be re-implemented either in the C++ operation
// or in more sophisticated CUDA kernels/wrappers.
