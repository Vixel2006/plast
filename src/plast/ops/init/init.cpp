...
#else
        throw std::runtime_error("CUDA is not enabled. Cannot create uniform tensor on CUDA device.");
#endif
    } else {
...
#else
        throw std::runtime_error("CUDA is not enabled. Cannot create tensor from data on CUDA device.");
#endif
    } else {
