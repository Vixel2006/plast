...
tensor::Tensor MinOperation::execute_cpu(const std::vector<const tensor::Tensor*>& inputs) const
{
    ...
    case core::DType::FLOAT32:
        if (full_reduction_)
        {
            plast_cpu_min_full_reduction_float(input.data_as<const float>(), output.data_as<float>(),
    ...
    case core::DType::INT32:
        if (full_reduction_)
        {
            plast_cpu_min_full_reduction_int32(input.data_as<const int32_t>(), output.data_as<int32_t>(),
                                         input.shape().data(), input.shape().size());
        }
        else
        {
            plast_cpu_min_reduction_dim_int32(input.data_as<const int32_t>(), output.data_as<int32_t>(),
                                        input.shape().data(), input.shape().size(),
                                        output.shape().data(), output.shape().size(), dim_);
        }
        break;
