...
tensor::Tensor ReluOperation::execute_cpu(const std::vector<const tensor::Tensor*>& inputs) const
{
    ...
    if (input_contiguous)
    {
        ...
        case core::DType::INT32:
            plast_cpu_relu_kernel_int32(output.data_as<int32_t>(),
                                         input.data_as<const int32_t>(), num_elements);
            break;
    ...
    else
    {
        ...
        {
        case core::DType::FLOAT32:
            plast_cpu_relu_kernel_strided_float(output.data_as<float>(),
                                                input.data_as<const float>(), output_shape,
                                                output.ndim(), input_strides);
            break;
        case core::DType::INT32:
            plast_cpu_relu_kernel_strided_int32(output.data_as<int32_t>(),
